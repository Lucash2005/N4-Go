    ...
    <div className="space-y-5">
      ...
      <section className="rounded-3xl border border-line bg-foam/60 p-4 sm:p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-ink-soft">
              確認今天發佈的修正清單（不是重跑全庫檢查）。字卡頁可「自動套用」Gemini 建議到本機；累積後按「匯出本機修正」貼給開發者即可併入正式版。
            </p>
          </div>
          <button
            type="button"
      ...
      <section className="rounded-3xl border border-line bg-foam/60 p-4 sm:p-5">
        ...
        <p className="mt-1 text-sm text-ink-soft">
          ...
          單字）。全庫完成前，線上單字只顯示現行 prompt（v2）已審過的卡；舊版待
          recheck 與未掃過的卡會進每日排程，審完才解鎖。文法與活用仍依本月路線，不受單字掃描擋住。
        </p>
        {scanProgress ? (
